...
class DBSCAN_CLS():
    ...
    """dbscan clustring for doc2vec vectors."""
    ...
    def read_vectors(self):
        self.w2v_coords = json.load(open(os.path.join(self._dir_path, '..' , 'data', 'sequence_coords_doc2vec.json')))
        self.w2v_vectors = json.load(open(os.path.join(self._dir_path, '..' , 'data', 'sequence_vectors_doc2vec.json')))
    ...
    def dbscan(self):
        ...
        for i, promoter in enumerate(self.modelVocab):  
            ...
            seq.update({"x_pca": float(self.X_principal['P1'][i])})
            seq.update({"y_pca": float(self.X_principal['P2'][i])})
